...
void	draw_mandlebrot(t_frame *frame)
{
	...
	z.x = 0.0;
	z.y = 0.0;
	c.x = scale(frame->x_win, frame, 0, WIN_WIDTH);
	c.y = scale(frame->y_win, frame, 0, WIN_HEIGHT);
...
void    draw_julia(double real, double imaginary, t_frame *frame)
{
	...
	frame->new_max = -2;
	frame->new_min = 2;
	z.x = scale(frame->x_win, frame, 0, WIN_WIDTH);
	z.y = scale(frame->y_win, frame, 0, WIN_HEIGHT);
	c.x = real;
	c.y = imaginary;
	while (i < MAX_ITER && ((z.x * z.x) + (z.y * z.y)) < 4.)
	{
